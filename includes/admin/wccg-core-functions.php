...
function wccg_generate_coupons( $number, $args = array() ) {
	...
	//$coupon_prefix = strip_tags($coupon_prefix);
...
function wccg_get_random_coupon($coupon_prefix = null) {
	...
	if(!empty($coupon_prefix)){
		$random_coupon = $coupon_prefix . '-' . strtoupper( $random_coupon );
	}else{
		$random_coupon = implode( '-', str_split( strtoupper( $random_coupon ), 4 ) );
	}
...
function wccg_ajax_process_batch_coupons() {
	...
	//Prefix Value
	$prefix_value = $post_data['coupon_prefix'];
